...
if _DB == 'MAFAT':
    ...
    # (Large vehicle) Truck, Light truck, Cement mixer, Dedicated agricultural vehicle, Crane truck, Prime mover, Tanker, Bus, Minibus.
    _C.DATA.CLASS_NAMES =['sedan', 'truck', 'dedicated agricultural vehicle', 'jeep', 'crane truck', 'prime mover',\
    ...
    # dataset - coco -----------------------
    _C.DATA.BASEDIR = '/media/Data/Data/Coco2014'
    _C.DATA.TRAIN = ['train2014']   # i.e., trainval35k
    _C.DATA.VAL = 'val2014'   # For now, only support evaluation on single dataset
    _C.DATA.NUM_CATEGORY = 80    # zvika - was 80 categories from COCO.
    #Categories are:
    _C.DATA.CLASS_NAMES = []  # NUM_CLASS (NUM_CATEGORY+1) strings, to be populated later by data loader. The first is BG.


# basemodel ----------------------
_C.BACKBONE.WEIGHTS = '/home/zvipe/Models/ObjectDetection/FasterRCNNTensorPack/tensorpack-master/examples/FasterRCNN/PreTrainedModels/ImageNet-R50-AlignPadding.npz'   # /path/to/weights.npz
...
_C.TRAIN.EVAL_PERIOD = 25 #zvika - was 25  # period (epochs) to run eva

# preprocessing --------------------
# Alternative old (worse & faster) setting: 600, 1024
_C.PREPROC.TRAIN_SHORT_EDGE_SIZE = [800, 800]  # [min, max] to sample from #Zvika - Kfir said maybe I need to pass on this one
...
# anchors ------------------------- #Zvika - need to change the anchors according to typical target sizes
_C.RPN.ANCHOR_STRIDE = 16
_C.RPN.ANCHOR_SIZES = (32, 64, 128, 256, 512)   # sqrt area of the anchor box
...
_C.RPN.TEST_POST_NMS_TOPK = 200   # if you encounter OOM in inference, set this to a smaller number
...
# fastrcnn training ---------------------
_C.FRCNN.BATCH_PER_IM = 512 # 512 #zvika - was 512
...
def finalize_configs(is_training):
    ...
    if is_training:
        ...
        os.environ['TF_CUDNN_USE_AUTOTUNE'] = '0' #zvika_added
