...
def get_train_dataflow():
    ...
    if _DB == 'COCO':
        roidbs = COCODetection.load_many(
            cfg.DATA.BASEDIR, cfg.DATA.TRAIN, add_gt=True, add_mask=cfg.MODE_MASK)
    if _DB == 'MAFAT':
        roidbs = mafatDetection.load_many(
            cfg.DATA.BASEDIR, cfg.DATA.TRAIN, add_gt=True, add_mask=cfg.MODE_MASK)
...
def get_eval_dataflow(shard=0, num_shards=1):
    ...
    """
    if _DB == 'MAFAT':
        roidbs = mafatDetection.load_many(cfg.DATA.BASEDIR, cfg.DATA.VAL, add_gt=False)
    elif _DB == 'COCO':
...
if __name__ == '__main__':
    ...
    #cfg.DATA.BASEDIR = os.path.expanduser([cfg.DATA.BASEDIR])
